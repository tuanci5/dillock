...
type TabType = 'overview' | 'strategy' | 'content' | 'channels' | 'website' | 'agent' | 'budget' | 'roadmap' | 'business';
...
const Sidebar = ({
  ...
  const baseMenuItems = [
    ...
    { id: 'agent', label: 'Marketing Đại lý', icon: <HeartHandshake className="w-5 h-5" /> },
    { id: 'budget', label: 'Ngân sách & Gói', icon: <BarChart3 className="w-5 h-5" /> },
    { id: 'business', label: 'Kế hoạch kinh doanh', icon: <Briefcase className="w-5 h-5" /> },
            ...
            {MARKETING_DATA.research.marketResearch.keywords.map((kw, i) => (
              <div key={i} className="group p-5 bg-slate-50 rounded-2xl border border-slate-100 hover:border-primary/50 hover:bg-white transition-all duration-300">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`text-[9px] font-bold px-2 py-1 rounded-md uppercase tracking-wider ${kw.competition.includes('Rất cao') ? 'bg-red-50 text-red-500' :
          ...
          <div className="flex bg-slate-100 p-2 rounded-2xl border border-slate-200 overflow-x-auto max-w-full">
            {[1, 2, 3, 4].map((w) => (
              <button
                ...
                className={`px-8 py-3 rounded-xl font-bold text-xs uppercase tracking-widest transition-all text-nowrap ${activeWeek === w ? 'bg-slate-900 text-primary shadow-xl' : 'text-slate-400 hover:text-slate-900'
                  }`}
              >
                Tuần {w}
              </button>
              ...
              >
                ...
                <div className="flex-1 space-y-4 text-center md:text-left">
                  <div className="flex flex-wrap justify-center md:justify-start gap-4">
                    <span className={`px-4 py-1.5 rounded-full text-[9px] font-bold uppercase tracking-widest border ${d.type === 'Bán lẻ' ? 'bg-amber-50 border-amber-200 text-amber-700' :
...
const WebsiteTab = () => (
  ...
  >
    ...
    <div className="p-12 white-card border-2 border-slate-100 relative overflow-hidden">
      ...
      <h3 className="text-2xl font-bold mb-10 flex items-center gap-4 text-slate-900 italic tracking-tighter">
        <div className="p-3 bg-slate-900 rounded-2xl text-primary"><Search className="w-6 h-6" /></div>
        Bảng Từ khóa SEO Chi tiết
      </h3>
      <div className="overflow-x-auto max-h-[800px] overflow-y-auto custom-scrollbar pr-2">
        <table className="w-full border-collapse">
          <thead className="sticky top-0 bg-white z-10">
            <tr className="border-b-2 border-slate-100 shadow-sm">
              <th className="py-5 px-6 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white">Từ khóa</th>
              <th className="py-5 px-6 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white">Volume/Tháng</th>
              <th className="py-5 px-6 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white">Độ khó</th>
              <th className="py-5 px-6 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest bg-white">Ý định</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {(MARKETING_DATA.websiteStrategy as any).seoKeywords?.map((kw: any, i: number) => (
              <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                <td className="py-6 px-6 text-sm font-bold text-slate-900 italic">{kw.keyword}</td>
                <td className="py-6 px-6 text-sm font-bold text-slate-900 text-center">{kw.volume}</td>
                <td className="py-6 px-6 text-center">
                  <span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase ${
                    kw.difficulty === 'Rất Cao' ? 'bg-red-100 text-red-600 border border-red-200' :
                    kw.difficulty === 'Cao' ? 'bg-red-50 text-red-500' :
                    kw.difficulty === 'Trung bình' ? 'bg-orange-50 text-orange-500' : 'bg-blue-50 text-blue-500'
                  }`}>
                    {kw.difficulty}
                  </span>
                </td>
                <td className="py-6 px-6 text-sm font-medium text-slate-500 text-center">{kw.intent}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    <div className="p-12 white-card border-2 border-slate-100 relative overflow-hidden">
      ...
      <h3 className="text-2xl font-bold mb-10 flex items-center gap-4 text-slate-900 italic tracking-tighter">
        <div className="p-3 bg-slate-900 rounded-2xl text-primary"><Megaphone className="w-6 h-6" /></div>
        Chiến dịch Google Ads (Search)
      </h3>
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead>
            <tr className="border-b-2 border-slate-100">
              <th className="py-5 px-6 text-left text-[10px] font-bold text-slate-400 uppercase tracking-widest">Từ khóa Ads</th>
              <th className="py-5 px-6 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">Giá thầu (CPC)</th>
              <th className="py-5 px-6 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">Đối sánh</th>
              <th className="py-5 px-6 text-center text-[10px] font-bold text-slate-400 uppercase tracking-widest">Nhóm Quảng cáo</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {(MARKETING_DATA.websiteStrategy as any).adsKeywords?.map((kw: any, i: number) => (
              <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                <td className="py-6 px-6 text-sm font-bold text-slate-900 italic">{kw.keyword}</td>
                <td className="py-6 px-6 text-sm font-bold text-emerald-600 text-center">{kw.cpc}</td>
                <td className="py-6 px-6 text-sm font-medium text-slate-500 text-center">{kw.matchType}</td>
                <td className="py-6 px-6 text-center">
...
const AgentTab = () => {
  const data = MARKETING_DATA.agentMarketing;
  return (
    <motion.div
      initial={{ opacity: 0, y: 30 }}
      animate={{ opacity: 1, y: 0 }}
      ...
      className="space-y-16"
    >
      <div className="p-14 white-card bg-slate-900 relative overflow-hidden group shadow-3xl">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(212,175,55,0.06),transparent_80%)] opacity-50" />
        <div className="relative z-10 space-y-6">
          <div className="p-4 bg-primary/10 rounded-2xl border border-primary/20 w-fit">
            <HeartHandshake className="w-10 h-10 text-primary" />
          </div>
          <h3 className="text-4xl font-bold text-white tracking-tighter italic">{data.title}</h3>
          <p className="text-xl text-slate-400 font-bold leading-relaxed italic border-l-4 border-primary pl-8 max-w-3xl">
            {data.strategy}
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        {data.supportActivities.map((act, i) => {
          const IconComp = act.icon === 'Facebook' ? Facebook : act.icon === 'Megaphone' ? Megaphone : MapPin;
          return (
            <div key={i} className="group p-10 white-card hover:border-primary/50 transition-all duration-500 shadow-xl">
              <div className="w-16 h-16 bg-slate-900 rounded-2xl flex items-center justify-center mb-10 group-hover:bg-primary transition-all shadow-xl">
                <IconComp className="w-8 h-8 text-primary group-hover:text-slate-950 transition-colors" />
              </div>
              <h4 className="text-2xl font-bold mb-4 text-slate-900 tracking-tight italic">{act.name}</h4>
              <p className="text-sm text-slate-500 mb-8 leading-relaxed font-medium italic">{act.desc}</p>
              <ul className="space-y-4">
                {act.items.map((item, idx) => (
                  <li key={idx} className="flex items-center gap-4 p-4 bg-slate-50 rounded-2xl border border-slate-100 group-hover:bg-white transition-colors">
                    <div className="w-2 h-2 rounded-full bg-primary shrink-0" />
                    <span className="text-xs font-bold text-slate-600 group-hover:text-slate-900 transition-colors italic tracking-tight">{item}</span>
                  </li>
                ))}
              </ul>
            </div>
...
const catColorMap: Record<string, { bg: string; text: string; border: string; dot: string }> = {
  ...
  "Đại lý & B2B": { bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200", dot: "bg-emerald-500" },
};
...
const BusinessTab = () => (
  ...
  >
    {/* Key Metrics Cards */}
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
      {MARKETING_DATA.businessPlan.metrics.map((metric: any, i: number) => {
        const IconComp = 
        ...
        return (
          <div key={i} className="p-10 white-card border-2 border-slate-100 hover:border-primary/50 transition-all duration-500 group shadow-2xl">
            <div className="flex items-center gap-4 mb-6">
              <div className="p-3 bg-slate-900 rounded-2xl text-primary group-hover:scale-110 transition-transform">
                <IconComp size={20} />
              </div>
            ...
            <div className="space-y-1">
              <p className="text-3xl font-bold text-slate-900 tracking-tighter italic">
                {metric.value || `${metric.min} - ${metric.max}`}
              </p>
              <p className="text-[10px] text-primary font-bold uppercase tracking-widest">{metric.unit}</p>
            </div>
          </div>
        );
      })}
    ...
    {/* Strategy Summary */}
    <div className="p-12 white-card bg-slate-900 relative overflow-hidden shadow-3xl">
      <div className="absolute top-0 right-0 p-12 opacity-5 text-white"><BarChart3 size={200} /></div>
      <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
        <div>
          <h3 className="text-3xl font-bold text-white mb-6 italic tracking-tighter">Bài toán Kinh doanh Dillock 2026</h3>
          <div className="space-y-6">
            <div className="flex items-start gap-5">
              ...
              <p className="text-slate-300 text-sm leading-relaxed">
                Dựa trên dự toán Marketing tối ưu <span className="text-white font-bold">(10% Doanh thu)</span>, Dillock hướng tới mục tiêu chiếm lĩnh thị trường khóa thông minh toàn quốc.
              </p>
            </div>
            <div className="flex items-start gap-5">
              <div className="w-10 h-10 rounded-xl bg-primary/20 flex items-center justify-center shrink-0 border border-primary/30">
              ...
              <p className="text-slate-300 text-sm leading-relaxed">
                Giá bán trung bình <span className="text-white font-bold">3.000.000 VNĐ/sản phẩm</span> đảm bảo tính cạnh tranh cực cao so với các đối thủ ngoại nhập như Kaadas hay Philips.
              </p>
            </div>
          </div>
        </div>
        <div className="p-10 bg-white/5 backdrop-blur-xl rounded-[3rem] border border-white/10 text-center">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-[0.4em] mb-4 italic">Tỷ lệ Phân bổ Marketing</p>
          <div className="relative inline-block">
             <p className="text-8xl font-display font-bold gold-text italic tracking-tighter">10%</p>
             <div className="absolute -top-4 -right-4 bg-emerald-500 text-slate-950 text-[9px] font-bold px-3 py-1 rounded-full uppercase">Optimal ROI</div>
          </div>
          <p className="text-xs text-slate-400 mt-6 font-bold uppercase tracking-widest italic leading-loose">Mức đầu tư an toàn cho giai đoạn chiếm lĩnh thị phần</p>
        </div>
      </div>
    </div>
    ...
    <div className="space-y-10">
      <h3 className="text-3xl font-bold text-slate-900 italic tracking-tighter text-center">Cơ cấu Doanh thu theo Kênh</h3>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {MARKETING_DATA.businessPlan.channels.map((ch: any, i: number) => (
          <div key={i} className="p-10 white-card border-2 border-slate-100 hover:border-primary/50 transition-all duration-500 group flex flex-col">
            <div className="flex justify-between items-start mb-8">
              <div className="w-16 h-16 bg-slate-50 rounded-[1.5rem] flex items-center justify-center text-slate-900 border border-slate-100 group-hover:bg-slate-900 group-hover:text-primary transition-all duration-500">
                <span className="text-2xl font-bold italic">{ch.share}%</span>
              </div>
              <span className="px-4 py-1.5 bg-slate-900 text-primary text-[10px] font-bold rounded-full uppercase tracking-widest shadow-xl">Target</span>
            </div>
            <h4 className="text-xl font-bold text-slate-900 mb-4 tracking-tight group-hover:text-primary transition-colors italic leading-tight">{ch.name}</h4>
            <p className="text-sm text-slate-500 mb-8 leading-relaxed font-bold italic h-12 overflow-hidden">{ch.strategy}</p>
            <div className="mt-auto pt-6 border-t border-slate-100">
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-1 opacity-60">Sản lượng Kỳ vọng</p>
              <p className="text-2xl font-display font-bold text-slate-900 italic tracking-tighter">{ch.target}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    {/* Profit Sharing Callout */}
    <div className="p-12 white-card border-2 border-primary/20 bg-primary/5 relative overflow-hidden group shadow-2xl">
      <div className="absolute top-0 right-0 p-8 opacity-5 text-primary group-hover:scale-110 transition-transform"><Award size={150} /></div>
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-10">
        ...
        <div className="flex-1 text-center md:text-left">
          <h4 className="text-2xl font-bold text-slate-900 mb-3 italic tracking-tighter">Ngưỡng Chia sẻ Lợi nhuận Đặc biệt</h4>
          <p className="text-sm text-slate-600 font-bold leading-relaxed italic uppercase tracking-tight">
            {MARKETING_DATA.businessPlan.profitSharing.note}
          </p>
        </div>
        <div className="p-8 bg-slate-900 rounded-[2.5rem] border border-primary/30 text-center min-w-[220px]">
          <p className="text-[10px] text-primary font-bold uppercase tracking-[0.3em] mb-2">Threshold</p>
          <p className="text-4xl font-display font-bold text-white italic tracking-tighter">1.500+</p>
          <p className="text-[10px] text-slate-500 font-bold uppercase mt-2">Sản phẩm</p>
        </div>
      </div>
    </div>

    {/* Growth Roadmap Summary */}
    <div className="p-16 bg-slate-50 rounded-[4.5rem] border border-slate-200 shadow-inner group">
      <h3 className="text-3xl font-bold mb-16 text-center text-slate-900 italic tracking-tighter">Lộ trình Tăng trưởng Doanh số</h3>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
        {MARKETING_DATA.businessPlan.growthPhases.map((phase: any, i: number) => (
          <div key={i} className="space-y-6 relative group/phase text-center">
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 p-2 bg-primary text-slate-950 text-[8px] font-bold rounded-lg uppercase tracking-widest z-20">Step 0{i+1}</div>
            <div className="w-20 h-20 bg-white rounded-[1.5rem] shadow-xl flex items-center justify-center mx-auto border border-slate-100 group-hover/phase:rotate-6 transition-transform">
               <Rocket className={`w-8 h-8 ${i === 0 ? 'text-blue-500' : i === 1 ? 'text-amber-500' : 'text-emerald-500'}`} />
            </div>
            <div>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1 italic">{phase.phase}</p>
              <h4 className="text-lg font-bold text-slate-900 mb-2 leading-tight italic tracking-tight">{phase.goal}</h4>
              <p className="text-xs text-slate-500 font-bold uppercase tracking-widest italic text-primary">{phase.focus}</p>
            </div>
          </div>
        ))}
      </div>
...
function AppContent() {
  ...
  const activeTab = useMemo(() => {
    ...
    return ['overview', 'strategy', 'content', 'website', 'channels', 'agent', 'budget', 'roadmap', 'business'].includes(part) ? part : 'overview';
  }, [pathname]);
